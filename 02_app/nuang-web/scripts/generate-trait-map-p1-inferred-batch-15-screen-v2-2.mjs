...
const outputPath = path.join(
  reviewDirectory,
  "TRAIT_MAP_P1_INFERRED_AXIS_INTERNAL_SCREEN_BATCH_15_V2_2.json",
);
const reportPath = path.join(
  docsDirectory,
  "89_P1_INFERRED_AXIS_INTERNAL_SCREEN_BATCH_15_V2_2.md",
);
...
const revisions = new Map([
  [
    "CAN-SCN-WORK-AFTERMATH-ATTENTION-SE-E",
    "부담스러운 업무가 지나간 뒤에는 함께한 사람들과 경험을 주고받는 동안 무엇이 남았고 다음에 무엇을 바꿀지 생각이 더 분명해지는 경향이 있다.",
  ],
  [
    "CAN-SCN-WORK-NEED-EXPRESSION-RESPONSE-SE-E",
    "담당자나 동료와 막힌 지점을 주고받는 동안 필요한 지원과 다음 행동을 더 분명히 정리하고, 대화 속에서 요청을 구체화하는 편이다.",
  ],
  [
    "CAN-SCN-WORK-SUCCESS-RESPONSE-SE-E",
    "성과에 관한 이야기를 함께 나누고 서로의 기여를 주고받는 동안 즐거움과 에너지가 커지며, 다음 아이디어도 더 잘 떠오르는 편이다.",
  ],
]);
  ...
  return {
    ...
    internalScreening: {
      ...
      rationale: revisedText
        ? "업무 영향·지원 요청·공동 축하만으로 E를 표시하지 않고, 상호작용 중 생각과 에너지가 움직이는 E를 직접 나타낸다."
        : "혼자 진행 과정과 생각을 정리한 뒤 필요한 사람과 소통하는 I의 흐름이 직접 드러난다.",
...
const report = {
  ...
  reportId:
    "TRAIT-MAP-P1-INFERRED-AXIS-INTERNAL-SCREEN-BATCH-15.0.1",
  batchId: batch.batchId,
  status:
    "P1_INFERRED_AXIS_BATCH_15_INTERNAL_SCREEN_COMPLETE_RECOMPOSITION_PENDING",
  publicationState: "research_only",
  ...
  entries,
  nextGate: {
    name: "BATCH_15_RECOMPOSITION_AND_BATCH_16_SCREEN",
    actions: [
      "3개 교정을 P1 누적 원장에 적용해 80개 이웃을 다시 검사한다.",
      "SM·partner 맥락의 P1-IAS-16을 판독한다.",
...
if (checkOnly) {
  ...
  if (stale) {
    console.error("v2.2 P1 inferred-axis batch 15 screen is stale.");
    process.exit(1);
  }
} else {
...
console.log(
  `P1 inferred-axis batch 15 v2.2: ${report.summary.variants} variants, ${report.summary.retainCandidates} retained, ${report.summary.revisionCandidates} revisions, independent approvals 0.`,
);

function buildMarkdown(result) {
  return `# v2.2 P1 추론 축 내부 판독 P1-IAS-15
