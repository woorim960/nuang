...
const chapterMeta = {
  1: {
    ...
    summary:
      "뉴앙 코드가 알려주는 모습과 알려주지 않는 것을 먼저 구분해요. 같은 ENAKQ라도 사람과 상황에 따라 모습이 달라질 수 있어요.",
    checkQuestion:
  ...
  3: {
    ...
    checkQuestion:
      "뉴앙 코드 중 가장 또렷하게 맞는 글자와 상황에 따라 달라지는 글자는 무엇인가요?",
  },
  4: {
    label: "함께 볼 때",
...
function simplifyCopy(copy) {
  return copy
    ...
    .replaceAll("대표 코드", "뉴앙 코드 코드")
    .replaceAll("대표 ENAKQ", "ENAKQ 뉴앙 코드")
    ...
    .replaceAll(
      "뉴앙 코드만으로 확정할 수 없는 결합 가설이에요",
      "뉴앙 코드만으로 확정할 수 없는 설명이에요",
    )
