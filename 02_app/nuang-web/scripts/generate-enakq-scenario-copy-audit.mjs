...
const inputPath = path.join(
  projectRoot,
  "docs/research/trait-map-data-center-v2/generated/ENAKQ_SCENARIO_REVIEW_V2.json",
);
const outputPath = path.join(
  projectRoot,
  "docs/research/trait-map-data-center-v2/generated/ENAKQ_SCENARIO_COPY_AUDIT_V2.json",
);
...
const report = {
  ...
  reportId: "ENAKQ-SCENARIO-COPY-AUDIT.0.1",
  ...
  nextGate:
    "자동 통과는 쉬운 한국어 승인이나 사실 정확성 승인이 아니다. 인지 인터뷰와 전문 검토를 계속 요구한다.",
  rows,
};
...
if (checkOnly) {
  if (
    ...
    console.error(
      "ENAKQ scenario copy audit is stale. Run npm run research:trait-map:v2:enakq-copy-audit.",
    );
    process.exit(1);
  }
} else {
...
console.log(
  `ENAKQ scenario copy audit: ${report.automaticPasses}/${report.auditedClaims} automatic passes, ${report.rewriteRequired} rewrites.`,
);
