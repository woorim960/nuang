...
const code = process.argv.find((argument) =>
  /^[EI][RN][GA][KM][CQ]$/.test(argument),
);
if (!code) {
  console.error(
    "Usage: node scripts/generate-derived-scenario-copy-audit.mjs INAKQ [--check]",
  );
  process.exit(1);
}
...
const inputPath = path.join(
  generatedDirectory,
  `${code}_SCENARIO_REVIEW_V2.json`,
);
const outputPath = path.join(
  generatedDirectory,
  `${code}_SCENARIO_COPY_AUDIT_V2.json`,
);
...
const rows = packet.claims.map((claim) => {
  ...
  if (assertion.length > 120) issues.push("모바일 한 문장 기준으로 너무 김");
  ...
  return {
    ...
    derivationMode:
      packet.lineage.find((item) => item.claimId === claim.claimId)
...
const report = {
  ...
  reportId: `${code}-SCENARIO-COPY-AUDIT.0.1`,
  ...
  nextGate:
    "자동 통과는 쉬운 한국어 승인이나 성향 판별력 승인이 아니다. 인지 인터뷰와 기준 성향 blind 비교를 계속 요구한다.",
  rows,
};
...
if (checkOnly) {
  if (
    ...
    console.error(
      `${code} copy audit is stale. Regenerate the derived copy audit.`,
    );
    process.exit(1);
  }
} else {
...
console.log(
  `${code} scenario copy audit: ${report.automaticPasses}/${report.auditedClaims} automatic passes, ${report.rewriteRequired} rewrites.`,
);
