...
현재 상태: `PACKET_READY_REVIEW_NOT_STARTED`
...
1. 역할별 자격 요건에 맞는 실제 검토자를 모집한다.
2. 신원·학위/면허 또는 연구 실적·이해관계·독립성을 확인한다.
3. 각 검토자에게 roster ID를 배정하고 동일한 역할 패킷의 복사본을 제공한다.
4. wave별 응답 파일을 잠그고 SHA-256과 수령 시각을 기록한다.
5. 응답 계약 검증과 정성 분석을 거쳐 claim별 결정을 조정한다.
6. 수정된 문장을 2030 한국어 사용자 인지 인터뷰로 보낸다.
