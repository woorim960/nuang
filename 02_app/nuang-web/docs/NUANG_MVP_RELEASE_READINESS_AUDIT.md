...
- 당시 샌드박스에서 기본 Turbopack 프로덕션 빌드가 CPU 사용 없이 장시간 정지해 Webpack으로 우회했다. 이후 Next.js 16.2.12·Node 25 환경에서 Webpack이 8GB heap을 소진한 반면 샌드박스 밖 기본 빌드는 정상 완료되어, 출시 빌드는 다시 지원되는 기본 컴파일러를 사용한다.
