...
import enakcNeighborPacket from "../../../docs/research/trait-map-data-center-v2/generated/ENAKC_NEIGHBOR_REVIEW_V2.json";
import irgmqNeighborPacket from "../../../docs/research/trait-map-data-center-v2/generated/IRGMQ_NEIGHBOR_REVIEW_V2.json";
...
const profiles = [
  {
    code: "ENAKC",
    changedDefinition: "ENAKC.general.definition.C",
    foundationClaims: enakcFoundationClaimsV2,
    neighborPacket: enakcNeighborPacket,
  },
  {
    code: "IRGMQ",
    changedDefinition: "IRGMQ.general.definition.Q",
    foundationClaims: irgmqFoundationClaimsV2,
    neighborPacket: irgmqNeighborPacket,
  },
] as const;

describe.each(profiles)(
  "$code Q/C-derived foundation and neighbors v2",
  ({ code, changedDefinition, foundationClaims, neighborPacket }) => {
    ...
    it("keeps claims unpublished, unique, and evidence-linked", () => {
      ...
      for (const claim of neighborPacket.claims) {
        ...
        expect(claim.publicationState).toBe("research_only");
        expect(
          claim.independentSourceRefs.every((id) => includedSourceIds.has(id)),
        ).toBe(true);
        expect(
