...
const report = JSON.parse(
  fs.readFileSync(
    path.join(
      process.cwd(),
      "docs/research/trait-map-data-center-v2/generated/TRAIT_MAP_CANONICAL_RECOMPOSITION_AUDIT_CAB_01_V2_1.json",
    ),
    "utf8",
  ),
);

describe("trait-map canonical recomposition CAB-01 v2.1", () => {
  it("recomposes all 93 corrected variants across 32 profiles", () => {
  ...
  it("keeps all five axes represented by passing neighbor edges", () => {
  ...
  it("does not promote structural safety to expert or customer approval", () => {
