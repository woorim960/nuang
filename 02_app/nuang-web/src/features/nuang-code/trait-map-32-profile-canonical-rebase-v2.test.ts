...
const report = JSON.parse(
  fs.readFileSync(
    path.join(
      process.cwd(),
      "docs/research/trait-map-data-center-v2/generated/TRAIT_MAP_32_PROFILE_CANONICAL_REBASE_V2.json",
    ),
    "utf8",
  ),
);

describe("trait-map 32-profile canonical rebase v2", () => {
  ...
  it("references the complete 713-variant canonical library without copying text", () => {
    expect(report.summary.referencedCanonicalVariants).toBe(713);
    for (const profile of report.profiles) {
  ...
  it("keeps the rebase research-only until review and publication gates pass", () => {
    ...
    expect(report.status).toContain("CONTENT_APPROVAL_PENDING");
  });
});
