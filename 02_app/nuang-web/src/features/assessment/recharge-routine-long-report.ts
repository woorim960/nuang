...
> = {
  ...
  gentle_reactivation: {
    ...
    levelCopy: {
      ...
      low: "이번에 답한 상황에서는 부담이 적은 활동으로 리듬을 바꾸는 행동이 드물게 나타났어요. 해야 할 일처럼 느껴지는 활동은 회복보다 압박을 더할 수 있어요. ‘운동해야지’처럼 큰 목표를 세우기보다 3~5분 걷기, 책상 한 칸 정리하기처럼 성공 여부를 따질 필요가 없는 작은 행동을 선택하면 시작 부담을 낮출 수 있어요.",
      middle:
