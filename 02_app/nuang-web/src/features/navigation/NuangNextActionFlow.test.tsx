...
describe("NuangNextActionFlow", () => {
  ...
  it("renders stable links for home and assessment entry points", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "결과가 쓰이는 순서" }),
    ).toBeInTheDocument();
    expect(screen.getByText("검사 후 흐름")).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "1단계 검사 열기" }),
    ).toHaveAttribute("href", "/home");
    expect(
      screen.getByRole("link", { name: "2단계 피드 열기" }),
    ).toHaveAttribute("href", "/feed");
    expect(
      screen.getByRole("link", { name: "3단계 성향지도 열기" }),
    ).toHaveAttribute("href", "/map");
    expect(
      screen.getByRole("link", { name: "4단계 내 리포트 열기" }),
