...
describe("privacy and no-go route metadata", () => {
  it.each(noindexRoutes)("%s keeps search indexing disabled", (_label, metadata) => {
  ...
  it.each(noindexRoutes)("%s keeps a product-quality page title", (_label, metadata) => {
    expect(typeof metadata.title).toBe("string");
    expect(metadata.title).toContain("NUANG");
    expect(metadata.title).not.toMatch(/MVP|payload|skeleton|preview|credential/i);
  });
});
