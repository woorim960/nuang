...
import { adminAdvertisingKillSwitchActionSchema } from "@/features/admin/admin-advertising-contract";
import {
  advertisingAdminActionError,
  toggleAdvertisingKillSwitch,
...
export async function POST(request: Request) {
  ...
  if (!payload.ok) return failure("중지 범위와 사유를 확인해 주세요.", 422);
  const response = await toggleAdvertisingKillSwitch({
    action: payload.data,
